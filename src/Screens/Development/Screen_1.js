...
import {View, Text, TouchableOpacity, StatusBar, SafeAreaView, ImageBackground, FlatList} from 'react-native'
    ...
    const Item = ({id, image, title, description, date, rated, comments, hasQR}) => {
        ...
        return(
            <View style={tw`shadow-md`}>
                <TouchableOpacity style={tw`h-auto self-stretch mx-2 mt-2 mb-${id === data.length ? isIphone ? 6 : 2 : 0} rounded-xl overflow-hidden shadow-md`} onPress={() => navigation.navigate('test_2', {title, image, description, date, rated, comments, hasQR, hasDecimal: hasDecimal})}>
